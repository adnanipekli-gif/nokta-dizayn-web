...
export async function ProjectProcessTimeline() {
  ...
  return (
    <section className="py-24 lg:py-32 bg-[#04101F] relative overflow-hidden" aria-labelledby="process-heading">
      ...
      <div className="section-container relative z-10">
        <FadeInOnScroll className="text-center mb-16 lg:mb-20">
          <p className="eyebrow text-[#11B5FF] mb-4">{t('eyebrow')}</p>
          <h2 id="process-heading" className="text-white max-w-2xl mx-auto">{t('title')}</h2>
          <p className="text-[#E9EEF3]/50 mt-4 max-w-lg mx-auto leading-relaxed">{t('subtitle')}</p>
        </FadeInOnScroll>
                ...
                <FadeInOnScroll key={step.number} delay={i * 60}>
                  <div className="group flex flex-col items-center text-center px-1">
                    <div className="relative z-10 w-[4.75rem] h-[4.75rem] rounded-full bg-[#071B34] border-2 border-[#11B5FF]/40 flex items-center justify-center mb-5 group-hover:border-[#11B5FF] group-hover:bg-[#11B5FF]/10 transition-all duration-300">
                      <span className="font-mono text-sm font-bold text-[#11B5FF]">{step.number}</span>
                    </div>
                    <h3 className="font-sora font-semibold text-white text-sm leading-snug mb-2 group-hover:text-[#11B5FF] transition-colors duration-200">{step.title}</h3>
                    <p className="text-xs text-[#E9EEF3]/45 leading-relaxed">{step.description}</p>
                  </div>
                </FadeInOnScroll>
              ))}
            </div>
